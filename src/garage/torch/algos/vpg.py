...
from garage.torch.algos import (_compute_advantages, _Default, _filter_valids,
                                _make_optimizer, _pad_to_last)
...
class VPG(BatchPolopt):
    ...
    def __init__(
        ...
        self._optimizer = _make_optimizer(optimizer,
                                          policy,
                                          lr=policy_lr,
                                          eps=_Default(1e-5))
    ...
    def _compute_loss(self, itr, paths, valids, obs, actions, rewards):
        ...
        baselines = torch.stack([
            _pad_to_last(self._get_baselines(path),
        ...
        advantages = _compute_advantages(self.discount, self._gae_lambda,
        ...
        if self._center_adv:
            means, variances = list(
                zip(*[(valid_adv.mean(), valid_adv.var())
                      for valid_adv in _filter_valids(advantages, valids)]))
        ...
        valid_objectives = _filter_valids(objective, valids)
    ...
    def process_samples(self, itr, paths):
        ...
        obs = torch.stack([
            _pad_to_last(path['observations'],
                         total_length=self.max_path_length,
                         axis=0) for path in paths
        ])
        actions = torch.stack([
            _pad_to_last(path['actions'],
                         total_length=self.max_path_length,
                         axis=0) for path in paths
        ])
        rewards = torch.stack([
            _pad_to_last(path['rewards'], total_length=self.max_path_length)
            for path in paths
        ])
