...
class TestTorchAlgoUtils(TfGraphTestCase):
    ...
    def test_compute_advantages(self, gae_lambda, rewards_val, baselines_val):
        ...
        torch_advs = torch_algo_utils._compute_advantages(
    ...
    def test_add_padding_last_1d(self):
        ...
        tensor_padding = torch_algo_utils._pad_to_last(nums_1d,
    ...
    def test_add_padding_last_2d(self):
        ...
        tensor_padding = torch_algo_utils._pad_to_last(nums_2d,
                                                       total_length=10)
    ...
    def test_add_padding_last_3d(self):
        ...
        tensor_padding = torch_algo_utils._pad_to_last(nums_3d,
                                                       total_length=10)
    ...
    def test_out_of_index_error(self, nums):
        ...
        with pytest.raises(IndexError):
            torch_algo_utils._pad_to_last(nums,
                                          total_length=10,
                                          axis=len(nums.shape))

    def test_make_optimizer_with_type(self):
        """Test _make_optimizer function with type as first argument."""
        optimizer_type = torch.optim.Adam
        module = torch.nn.Linear(2, 1)
        lr = 0.123
        optimizer = torch_algo_utils._make_optimizer(optimizer_type,
                                                     module,
                                                     lr=lr)
        assert isinstance(optimizer, optimizer_type)
        assert optimizer.defaults['lr'] == lr

    def test_make_optimizer_with_tuple(self):
        """Test _make_optimizer function with tuple as first argument."""
        optimizer_type = (torch.optim.Adam, {'lr': 0.1})
        module = torch.nn.Linear(2, 1)
        optimizer = torch_algo_utils._make_optimizer(optimizer_type, module)
        assert isinstance(optimizer, optimizer_type)
        assert optimizer.defaults['lr'] == optimizer_type[1]['lr']

    def test_make_optimizer_raise_value_error(self):
        """Test _make_optimizer raises value error."""
        optimizer_type = (torch.optim.Adam, {'lr': 0.1})
        module = torch.nn.Linear(2, 1)
        with pytest.raises(ValueError):
            _ = torch_algo_utils._make_optimizer(optimizer_type,
                                                 module,
                                                 lr=0.123)
